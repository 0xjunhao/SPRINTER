...
class TokenAcceptanceModel(torch.nn.Module):
    def __init__(self, input_size, tokenizer, gpt_neo_model_name='EleutherAI/gpt-neo-125m'):
        ...
        self.gpt_neo = transformers.AutoModelForCausalLM.from_pretrained(
            gpt_neo_model_name)
...
def speculative_sampling_sprinter(prefix: torch.Tensor, approx_model: torch.nn.Module, target_model: torch.nn.Module, DRTmodel: torch.nn.Module,
    ...
    while prefix.shape[1] < T:
        ...
        while True:
            ...
            x, total_draft_model_time, hidden_states_last_layer = approx_model_cache.generate(
        ...
        if n < prefix_len + candidate_length - 1:
            ...
            start = time.time()
            t = sample(max_fn(
                target_model_cache._prob_history[:, n, :] - approx_model_cache._prob_history[:, n, :]))
            resample_SD = time.time() - start
            if verbose:
                print(
                    f"target resamples at position {n}: \033[34m{Decoder().decode(t)}\033[0m")
            resample_count += 1
            ...
            if verbose:
                print(
                    f"target samples {n}: \033[35m{Decoder().decode(t)}\033[0m")
...
if __name__ == "__main__":
    ...
    DRTmodel_sprinter = TokenAcceptanceModel(
    ...
    for name in name_list:
        ...
        for prompt_single in prompts:
            ...
            if name == 'sprinter':
                output, time_SD, avg_token = speculative_sampling_sprinter(
                    input_ids, draft, target, DRTmodel_sprinter, max_len, gamma=gamma, top_k=top_k, top_p=top_p, random_seed=random_seed, verbose=verbose)
            end_sample_time = time.time()
            avg_token_list.append(avg_token)
            single_sampling_time.append(end_sample_time - begin_sample_time)
            time_vec_SD += time_SD
            dic = {'overall_time': [sum(single_sampling_time)], 'time_vec_SD': [time_vec_SD], 'avg_single_run_token': [sum(avg_token_list)/len(
                avg_token_list)], 'avg_time': [sum(single_sampling_time)/len(single_sampling_time)], 'std_time': [np.std(single_sampling_time)]}
            pd.DataFrame(dic).to_csv('lm1b_neo_time_' +
                                     str(name)+'.csv', index=False)
            if name == 'sprinter':
                sprinter_collection.append({'prompt': prompt, 'res': tokenizer.decode(output[0], skip_special_tokens=True)[
                                           len(prompt):], 'response': prompt + tokenizer.decode(output[0], skip_special_tokens=True)[len(prompt):]})
            prompt_counter += 1
